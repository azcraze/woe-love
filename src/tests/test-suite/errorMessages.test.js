...
describe('errors', () => {
  ...
  test('should have correct error messages', () => {
    expect(errors.typeChecks[0]).toBe('a valid hex color code. use `>>hex` for a quick reference');
    ...
    expect(errors.dicsordChecks[1]).toBe('is not a valid channel ID or mention');
  });
});

describe('errMsg', () => {
  test('should return correct error message for given key and index', () => {
    expect(errMsg('types', 0)).toBe('**Error:** a valid hex color code. use `>>hex` for a quick reference');
    expect(errMsg('argCount', 2)).toBe('**Error:** requires 2 arguments');
    expect(errMsg('info', 1)).toBe('**Error:** No answer. Command has timed out.');
    expect(errMsg('invalidInput', 3)).toBe('**Error:** Invalid Input');
    expect(errMsg('dicsordChecks', 0, 'Oops!')).toBe('Oops! is not a valid channel ID or mention');
  });
});
