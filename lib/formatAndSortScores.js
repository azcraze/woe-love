...
function transformScores(scoreboard, input) {
  ...
  if (
    !scoresObject ||
    "object" !== typeof scoresObject ||
    Array.isArray(scoresObject)
  ) {
  ...
  log(
    `Transformation completed. Returning scores object with ${
      Object.keys(transformedScores).length
    } players`,
  );
...
function sortScores(scoresObject) {
  ...
  const sortedArray = Object.entries(scoresObject).sort(
